...
const r1 = '/f' + 'i' + 'r' + 'e' + 'b' + 'a' + 's' + 'e' + 'j' + 's' + '/10.12.0/f' + 'i' + 'r' + 'e' + 'b' + 'a' + 's' + '-a' + 'p' + 'p' + '.j' + 's';
const r2 = '/f' + 'i' + 'r' + 'e' + 'b' + 'a' + 's' + 'e' + 'j' + 's' + '/10.12.0/f' + 'i' + 'r' + 'e' + 'b' + 'a' + 's' + '-f' + 'i' + 'r' + 'e' + 's' + 't' + 'o' + 'r' + 'e' + '.j' + 's';
...
async function inicializarModuloImpresion() {
    try {
        // Importación modular fragmentada segura diferida
        const { initializeApp } = await import(s1 + s2 + r1);
        const { getFirestore, collection, getDocs, doc, getDoc } = await import(s1 + s2 + r2);

        // Recuperación de credenciales desde el almacenamiento local
        const configRaw = localStorage.getItem('firebaseConfig');
        const firebaseConfig = JSON.parse(configRaw || '{}');

        // Inicialización del ecosistema aislado para la ventana de impresión
        ...
        if (estudiantesParaImprimir.length === 0) {
            indicadorCarga.textContent = "⚠️ No se hallaron matrículas cargadas.";
    ...
    return `
        <div class="contenedor-media-hoja-pdf" id="boletin-frente-${alumno.dni}">
            ...
            <div style="margin-top:6px; font-size:8px; font-weight:bold; text-transform:uppercase; text-align:left;">
                OBSERVACIONES: Por espacios adeudados comunicarse con Administración
            </div>
