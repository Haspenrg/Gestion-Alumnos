...
function inicializarEventos() {
  ...
  if (domElements.btnConfirmarCarga) domElements.btnConfirmarCarga.addEventListener("click", ejecutarImportacionFinal);
  ...
  if (domElements.btnCerrarImpresion) domElements.btnCerrarImpresion.addEventListener("click", cerrarConsolaImpresion);
  ...
  if (domElements.confirmBtnAceptar) domElements.confirmBtnAceptar.addEventListener("click", aceptarConfirmacionHaspen);
...
// Event Listeners para los botones del sub-modal PPI
document.addEventListener("DOMContentLoaded", () => {
  const btnCerrar = document.getElementById("btnCerrarObsPPI");
  const btnGuardar = document.getElementById("btnGuardarObsPPI");

  if (btnCerrar) {
    btnCerrar.addEventListener("click", cerrarModalObsPPI);
  }

  if (btnGuardar) {
    btnGuardar.addEventListener("click", guardarModalObsPPI);
  }

  inicializarEventos();
  const usuarioActual = { role: "admin", hasWritePermission: true };
  verificarPermisosCSV(usuarioActual);
});
