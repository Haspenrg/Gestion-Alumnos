...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    // 5. NUEVO: Escuchadores reactivos para la barra de auditoría masiva e interactiva
...
function gestionarPanelesFormulario() {
    ...
    const rol = selectRolElement.value ? selectRolElement.value.toLowerCase().trim() : "";
...
function agregarCatedraProfesorBolsa() {
    ...
    try {
        ...
        if (docentesAsignados.length > 0) {
            // Control coercitivo institucional: No se permiten dos titulares concurrentes
            ...
            // Advertencia interactiva de Co-titularidad o Suplencias escalonadas
            const listaDetalle = docentesAsignados.map(d => `• ${d.nombre} (${d.revista})`).join("\n");
            const autorizar = confirm(
                `⚠️ AUDITORÍA EN CALIENTE - DETECCIÓN DE MULTI-DOCENTES:\n\n` +
        ...
        console.error("Error en validación institucional de revista:", e);
    }
...
async function renderizarTablaUsuarios() {
    ...
    let usuariosFiltrados = usuarios.filter(user => {
        ...
        // Si se selecciona auditar superpoblación, se aíslan los multi-docentes
        if (modoAuditoria === "SUPERPOBLADO") {
            if (!user.bolsaHoras || user.bolsaHoras.length === 0) return false;
            const tieneMateriaSuperpoblada = user.bolsaHoras.some(h => {
                const materiaPura = h.replace(/\[.*?\]\s*/, "").trim();
    ...
    usuariosFiltrados.forEach(user => {
        ...
        if (user.bolsaHoras && user.bolsaHoras.length > 0) {
            const liMaterias = user.bolsaHoras.map(h => {
                let estiloColor = "color: #0d9488; font-weight:600;"; // Verde para titular
